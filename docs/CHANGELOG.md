...
### 🔧 Fixed: Map Description Update & UX Improvements

**Problem Solved:**
Map description content was generated by AI but not being inserted into WordPress pages. Also improved UX for preview & publish mode with loading states and link visibility.
...
- **Solution:** Changed to flexible matching pattern (`cssId.includes('map') && cssId.includes('description')`)
  ...
  - File: `src/lib/elementor-replacer.ts:175-193`

- **Sample Page API:** Also updated with same fix + debug logging
  - File: `src/app/api/sample-page/route.ts:316-343`
...
- ✅ **Flexible CSS ID matching** for Elementor widgets

### Technical Details
- Map description fix applies to all generation modes: direct, preview & publish, sample pages
