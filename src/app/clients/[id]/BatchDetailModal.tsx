...
export default function BatchDetailModal({ batchId, onClose }: BatchDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                          ...
                          <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td className="px-4 py-3 text-sm">
                              {page.generatedContent ? (
                                <button
                                  onClick={() => setViewingContentPageId(page.id)}
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-lg transition-colors text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20"
                                >
      ...
      {/* Preview-style read-only content viewer */}
      {viewingContentPageId && batch && (
        <BatchContentModal
          pages={batch.generatedPages.filter((p) => p.generatedContent)}
          initialPageId={viewingContentPageId}
          onClose={() => setViewingContentPageId(null)}
        />
      )}
    </div>
  );
